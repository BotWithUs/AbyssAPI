...
public class LocalPathing {
    ...
    public static int getLocalStepsTo(Vector3i start, int srcSize, RouteStrategy strategy, boolean findAlternative) {
    ...
    public static boolean isTileReachable(WorldTile dest) {
        ...
        return getLocalStepsTo(player.getGlobalPosition(), 1, new FixedTileStrategy(dest), false) > -1;
    }
    ...
    public static boolean isNpcReachable(Npc npc) {
        ...
        CacheNpc def = Cache.getNpc(npc.getId());
        return getLocalStepsTo(player.getGlobalPosition(), def.getSize(), new EntityStrategy(npc), true) > -1;
    }

    public static boolean isObjectReachable(SceneObject obj) {
        Player player = Players.self();
        if (player == null) {
            return false;
        }
        return getLocalStepsTo(player.getGlobalPosition(), 1, new ObjectStrategy(obj), true) > -1;
    }

    public static List<WorldTile> findLocalRoute(Vector3i start, int srcSize, RouteStrategy strategy, boolean findAlternative) {
        isAlternative = false;
        ...
        boolean found = switch (srcSize) {
            case 1 -> checkSingleTraversal(start.getX(), start.getY(), strategy);
            case 2 -> checkDoubleTraversal(start.getX(), start.getY(), strategy);
            default -> checkVariableTraversal(start.getX(), start.getY(), srcSize, strategy);
        };
